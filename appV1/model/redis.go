...
func GetAllRedis(pageNumber int) []*BookInfo {
	...
	//var book *BookInfo
	userData, err := redisClient.HGet("GetAllMap", pageNumberStr).Result()
	if err != nil {
		...
		err = redisClient.HSet("GetAllMap", pageNumberStr, bookJson).Err()
		if err != nil {
...
func SaveBooksToRedis(books []*BookInfo) error {
	...
	key := "books"
	existingData, err := redisClient.HGetAll(key).Result()
