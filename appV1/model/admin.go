...
func GetUserInformationRedis(userId int64) *User {
	...
	var user *User
	userData, err := redisClient.HGet("GetUserInformationMap", userIdStr).Result()
	if err != nil {
		...
		err = redisClient.HSet("GetUserInformationMap", userIdStr, userJson).Err()
		if err != nil {
...
func DeleteBookRedis(bookId int64) int {
	...
	bookNameString, err := redisClient.HGet("BookIdMap", bookIdString).Result()
	if err != nil {
		fmt.Printf("查询bookNameString失败")
	}
	_, _ = redisClient.LRem("BookNameList", 0, bookNameString).Result()
	_, _ = redisClient.HDel("books", bookNameString).Result()
	_, _ = redisClient.HDel("BookIdMap", bookIdString).Result()
