...
func GetBookByNameRedis(bookName string) *BookInfo {
	...
	var book *BookInfo
	bookData, err := redisClient.HGet(context.Background(), "books", bookName).Result()
	fmt.Println("qian")
	...
	if bookData == "" {
		...
		if book != nil {
			...
			err = redisClient.HSet(context.Background(), "books", bookName, string(bookJSON)).Err()
			if err != nil {
...
func AutomaticLogin(code string) *User {
	user := &User{}
	userData, err := redisClient.HGet(context.Background(), "QrcodeMap", code).Result()
	if err != nil {
		fmt.Printf("获取失败")
		return nil
	}
