...
func LibrarianLogin(c *gin.Context) {
	...
	if dbUser.Id > 0 {
		fmt.Println(dbUser.UserName)
		sessionID, err := model.GenerateSessionID() //生成的sessionID是string类型的
		if err != nil {
			fmt.Println("sessionId生成失败")
		}
		...
		err = model.SetSession(sessionID, values, expiration)
		// 设置会话ID的Cookie
...
func GetUserInformation(c *gin.Context) {
	...
	ret := model.GetUserInformationRedis(statusId)
	if ret.Id > 0 {
		c.JSON(http.StatusOK, tools.HttpCode{
			Code:    tools.OK,
			Message: "",
...
func DeleteBook(c *gin.Context) {
	...
	model.DeleteBookRedis(statusId)
	if ret == 1 {
		c.JSON(http.StatusOK, tools.HttpCode{
			Code:    tools.OK,
			Message: "查询数据库有问题",
